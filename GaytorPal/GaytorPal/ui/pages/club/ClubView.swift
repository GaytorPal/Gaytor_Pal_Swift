            ...
            ScrollView {
                VStack(alignment: .leading) {
                    ...
                    if let category = selectedCategory, let _ = categories[category] {
                        ...
                        FilteredClubView(clubs: clubVM.clubs.filter { $0.categories.contains(category) })
                            .padding(.top)
                        ...
                        SectionView(sectionTitle: "Organizations", clubs: clubVM.clubs)
                        SectionView(sectionTitle: "Clubs", clubs: clubVM.clubs)
                        SectionView(sectionTitle: "New Orgs", clubs: clubVM.clubs)
                    }
                }
            }
